...
// Tool: Fetch cryptocurrency data using CoinGecko API
async function cryptoCurrency({ coin }) {
  const response = await fetch(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=inr&ids=${coin}`);
  const data = await response.json();
  return data;
...
// Tool: Fetch current weather data using WeatherAPI
async function weatherInformation({ city }) {
  const response = await fetch(`http://api.weatherapi.com/v1/current.json?key=590c498a569c4497b34194643251912&q=${city}&aqi=no`);
  const data = await response.json();
  return data;
...
// Define tool metadata for Gemini to understand function capabilities and parameters
const cryptoInfo = {
  name: "cryptoCurrency",
  description: "Provides current price or other information for cryptocurrencies like Bitcoin, Ethereum, etc.",
  parameters: {
    type: Type.OBJECT,
    properties: {
      coin: {
...
const weatherInfo = {
  name: "weatherInformation",
  description: "Provides current weather information for a given city (e.g., London, Goa, etc.)",
  parameters: {
    type: Type.OBJECT,
    properties: {
      city: {
...
async function runAgent() {
    while (true) {
        ...
        // If Gemini returns a final text response
        History.push({
            role: "model",
            parts: [{ text: result.text }]
        });
